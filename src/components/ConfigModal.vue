    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <p class="text-xs text-base-content/50 mt-2">
          Pour utiliser ton propre quota Gemini. Stockee de maniere securisee.
        </p>
        <div v-if="hasKey()" class="mt-2 flex items-center gap-2 text-xs text-success">
          <span>✓</span>
          <span>Cle API configuree</span>
        </div>
        <div v-else class="mt-2 flex items-center gap-2 text-xs text-base-content/50">
          <span>ℹ️</span>
          <span>Sans cle, l'API partagee est utilisee</span>
        </div>
      </div>
    </div>
